...
const LS = "ls";
const CD = "cd";
const COMMAND_INDICATOR = "$";
const DIR = "dir";
const BACK = "..";


const isChangeDirectory = (instruction) => {
	return instruction.startsWith(COMMAND_INDICATOR) && instruction.includes(CD);
};

const isListDirectory = (instruction) => {
	return instruction.startsWith(COMMAND_INDICATOR) && instruction.includes(LS);
};

const isDirectory = (instruction) => {
	return instruction.split(" ")[0] === DIR
};

const getFileSize = (instruction) => {
	return parseInt(instruction.split(" ")[0]);
};

const getDirName = (instruction) => {
	return instruction.split(" ")[1];
};

const getDirNameFromCommand = (instruction) => {
	return instruction.split(" ")[2];
};

const computeFullPath = (navDirs) => {
    return navDirs.length ? "/" + navDirs.join("/") : "/";
};

const buildDirs = (input) => {
	const navDirs = [];
	const dirsMap = {"/": {size: 0, dirs: []}};

	input.forEach(instruction => {
		if (isChangeDirectory(instruction)) {
			const dirName = getDirNameFromCommand(instruction);
			if (dirName === ROOT) {
				navDirs.length = 0;
			} else if (dirName === BACK) {
				navDirs.pop();
			} else {
				navDirs.push(dirName);
			}

		} else if (isListDirectory(instruction)) {
			// do nothing
		} else if (isDirectory(instruction)) {
			const newDirName = getDirName(instruction);
            const fullPath = computeFullPath(navDirs);
            const fullPathNewDir = fullPath.length > 1 ? fullPath + "/" + newDirName : fullPath + newDirName;
            const fullPathCurrentDir = computeFullPath(navDirs);
			if (!dirsMap[fullPathNewDir]) {
				dirsMap[fullPathNewDir] = {size: 0, dirs: []};
            }
            dirsMap[fullPathCurrentDir].dirs.push(fullPathNewDir);
        } else {
            const fullPathCurrentDir = computeFullPath(navDirs);
            const fileSize = getFileSize(instruction);
            dirsMap[fullPathCurrentDir].size += fileSize;
        }
...
const computeTotalDirSizes = (dirsMap, asObj) => {
    const paths = Object.keys(dirsMap);
    const values = paths.reduce((acc, currentPath, index) => {
        const entries = Object.entries(dirsMap);
        entries.forEach(entry => {
            if (entry[0].startsWith(currentPath)) {
                if (!acc[asObj ? currentPath : index]) {
                    acc[asObj ? currentPath : index] = 0;
                }
                acc[asObj ? currentPath : index] += entry[1].size;
            }
        });
        return acc;
    }, asObj ? {} : []);
...
const filterDirSizeSmallerOrEqualTo = (input, maxVal) => {
    return computeTotalDirSizes(buildDirs(input)).filter(size => size <= maxVal);
};

const sumSelectedTotalSizes = (input, maxVal) => filterDirSizeSmallerOrEqualTo(input, maxVal).reduce((acc, val) => acc += val, 0);

// Puzzle 1
// Results puzzle 1
console.log("Part 1 example: ", sumSelectedTotalSizes(exampleInput, 100000));
console.log("Part 1 real: ", sumSelectedTotalSizes(realInput, 100000));
...
const sortDirBySize = (input) => {
	const entries = Object.entries(computeTotalDirSizes(buildDirs(input), true))
	return entries.sort((a, b) => a[1] - b[1]);
}

const computeTotalDiskSizeTaken = (input) => computeTotalDirSizes(buildDirs(input))[0];

const retrieveSizeOfDirToRemove = (input) => {
	const totalDiskSizeTaken = computeTotalDiskSizeTaken(input);
	const sortedDirs = sortDirBySize(input);
	const diskToRemoveArr = sortedDirs.filter(elem => FREE_SPACE_NEEDED <= TOTAL_DISK_SPACE - totalDiskSizeTaken + elem[1]);
	const diskToRemoveObj = diskToRemoveArr[0];
	return diskToRemoveObj[1];
};

console.log("Part 2 example: ", retrieveSizeOfDirToRemove(exampleInput));
console.log("Part 2 real: ", retrieveSizeOfDirToRemove(realInput));
