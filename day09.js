...
const moveTail = (tailPos, headPos) => {
	if (tailPos[0] < headPos[0] && tailPos[1] < headPos[1]) {
		tailPos[0]++;
		tailPos[1]++;
	} else if (tailPos[0] > headPos[0] && tailPos[1] > headPos[1]) {
		tailPos[0]--;
		tailPos[1]--;
	} else if (tailPos[0] < headPos[0] && tailPos[1] > headPos[1]) {
		tailPos[0]++;
	...
	const visitedPosition = [...tailPos];
	return visitedPosition.join(":");
}

const moveHeadAndTail = (input) => {
	const headPos = [0, 0];
	const tailPos = [0, 0];
	const posVisitedByTail = ["0:0"];
	const sequence = transformSequence(input);
	while (sequence.length > 0) {
		moveHead(sequence, headPos);
		if (areAdjacent(tailPos, headPos)) {
			continue;
		} else {
			const visitedPosition = moveTail(tailPos, headPos);
			posVisitedByTail.push(visitedPosition);
		}
	}
	return new Set(posVisitedByTail);
}

const getNumberOfVisitedPositionsByTail = (input) => {
	const visitedPositions = moveHeadAndTail(input);
	return visitedPositions.size;
}

// Results puzzle 1
console.log("Part 1 example: ", getNumberOfVisitedPositionsByTail(exampleInput));
console.log("Part 1 real: ", getNumberOfVisitedPositionsByTail(realInput));


// Puzzle 2
const moveHeadAndTailPart2 = (input) => {
	...
	while (sequence.length > 0) {
		pos.forEach((position, index) => {
			if (index === 0) {
				moveHead(sequence, position);
			}
			if (index < 9) {
				if (index < 8 && !areAdjacent(pos[index+1], position)) {
					moveTail(pos[index+1], position);
				} else if (index === 8 && !areAdjacent(pos[index+1], position)) {
					const visitedPosition = moveTail(pos[index+1], position);
					posVisitedByTail.push(visitedPosition);
				}
			}
		});
...
const getNumberOfVisitedPositionsByTailPart2 = (input) => {
	const visitedPositions = moveHeadAndTailPart2(input);
	return visitedPositions.size;
}


// Results puzzle 2
console.log("Part 2 example: ", getNumberOfVisitedPositionsByTailPart2(exampleInput));
console.log("Part 2 larger example: ", getNumberOfVisitedPositionsByTailPart2(largerExampleInput));
console.log("Part 2 real: ", getNumberOfVisitedPositionsByTailPart2(realInput));
